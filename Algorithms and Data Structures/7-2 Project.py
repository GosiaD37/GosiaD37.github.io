...
def build_query(filter_type):
    if filter_type == 'WR':
        ...
        return {
            '$or': [
                {'$and': build_query('WR')['$and']},
                {'$and': build_query('MWR')['$and']},
                {'$and': build_query('DRIT')['$and']}
            ]
        }
    else:
        return {}
...
def update_pie_chart(filter_type):
    ...
    title = rescue_titles.get(filter_type, 'Dog Breed Distribution')

    # Clean breed data: Trim spaces and standardize capitalization
    df['breed'] = df['breed'].str.strip().str.title()

    if filter_type == 'RESET':
        # For RESET, show the distribution of all breeds
...
def update_map(view_data, selected_rows):
    ...
    # Check if the necessary columns for location exist
    if 'location_lat' not in df.columns or 'location_long' not in df.columns:
        return "Location data not available for display."

    # If no rows are selected, handle it gracefully
    if not selected_rows:
        return "No rows selected for map display."
    
    # Prepare markers for multiple selected rows
    markers = []
    for row_idx in selected_rows:
        # Ensure we have a valid index for the row
        if row_idx < len(df):
    ...
    if markers:
        # Use the first selected row for the center position
